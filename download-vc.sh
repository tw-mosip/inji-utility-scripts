...
if [[ -z ${UIN_VID} ]];then
    echo "Usage: "
    echo "1. ./download-vc.sh <UIN/VID>"
    echo "2. ./download-vc.sh <UIN/VID> <ENV>"
    echo ""
    echo "Example: "
    echo "# Without env argument, defaults to dev2  -->  ./download-vc.sh 3718934652"
    echo "# With env argument                       -->  ./download-vc.sh 3718934652 dev2 "
    exit 1
fi
...
echo "Generated Random Tx Id: $TX_ID"
...
echo "Received RequestId: $REQUEST_ID"
...
while
    ...
    echo $CRED_STATUS_RESP

    STATUS_CODE=`echo $CRED_STATUS_RESP|jq '.response.statusCode'`

    echo "Status of VC Issuance: ${STATUS_CODE}"

    if [[ ${STATUS_CODE} = "\"FAILED\"" ]];then
        echo "Unable to generate VC"
        exit 1
    fi
...
echo "VC issue Completed, Downloading VC"
